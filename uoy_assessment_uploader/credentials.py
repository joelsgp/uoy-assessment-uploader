...
def ensure_password(username: str, password: Optional[str], use_keyring: bool) -> str:
    ...
    The extra arguments passed to :func:`ensure_credential` are :const:`KEYRING_NAME_PASSWORD` for ``keyring_name``,
...
def ensure_exam_number(
    ...
    The extra arguments passed to :func:`ensure_credential` are :const:`KEYRING_NAME_EXAM_NUMBER` for ``keyring_name``,
...
def ensure_credential(
    ...
    :param username: username passed through to :module:`keyring`
    :param credential: either None, or the secret passphrase to be returned.
    :param use_keyring: whether to use :module:`keyring` to save the credential.
        If this is True, this function will attempt to retrieve the credential with :func:`keyring.get_password`.
    ...
    :param keyring_name: concatenated with :var:`__name__` to get the ``service_name`` to pass to :mod:`keyring`
    :param prompt: prompt for :func:`getpass` to use, if the credential is not retrieved from the keyring.
